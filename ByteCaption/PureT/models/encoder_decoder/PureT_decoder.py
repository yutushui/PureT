...
class Decoder(nn.Module):
    def __init__(
        ...
        # Dynamic position encoding based on SEQ_LEN from config
        max_seq_len = int(getattr(cfg.MODEL, 'SEQ_LEN', 100)) + 10  # Add some margin
        self.pos_embed = nn.Embedding.from_pretrained(
            sinusoid_encoding_table(max_seq_len, self.embed_dim, 0), freeze=True
        )
    ...
    def init_buffer(self, batch_size, device='cpu'):
        ...
        for layer in self.layers:
            layer.init_buffer(batch_size, device)
    ...
    def forward(self, gx, seq, encoder_out, seq_mask=None, att_mask=None):
        ...
        seq_len = seq.size()[1]
        device = seq.device
        pos_indx = torch.arange(1, seq_len + 1, device=device).view(1, -1)
        if self.seq_len is not None:
            seq_len = self.seq_len + seq_len
            self.seq_len = seq_len
            pos_indx = torch.arange(seq_len, seq_len + 1, device=device).view(1, -1)
...
class DecoderLayer(nn.Module):
    ...
    def init_buffer(self, batch_size, device='cpu'):
        self.word_attn.init_buffer(batch_size, device)
    ...
    def forward(self, gx, x, encoder_out, seq_mask, att_mask=None):
        ...
        if self.use_gx:
            ...
            if att_mask is not None:
                ...
                _att_mask = torch.cat(
                    [att_mask, torch.ones(att_mask.size(0), device=device).unsqueeze(1).unsqueeze(1)], 2
                ).long()
            else:
                _att_mask = None
        else:
...
class MultiHeadSelfAttention(nn.Module):
    ...
    def init_buffer(self, batch_size, device='cpu'):
        # [B, nH, 0, C/nH]
        self.buffer_key = torch.zeros((batch_size, self.num_heads, 0, self.head_dim), device=device)
        self.buffer_value = torch.zeros((batch_size, self.num_heads, 0, self.head_dim), device=device)
