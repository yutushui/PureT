...
使用方法:
    python generate_coco_vocab.py --split train --output data/coco/coco_vocabulary.txt
"""
...
def load_coco(split='train'):
    path = '/root/autodl-fs/AbdoTW___coco_2014'
...
def build_vocabulary(split='train', max_samples=None, vocab_size=None):
    """
    从COCO数据集构建词汇表
    
    Args:
        split: 数据集分割 ('train' - COCO只有train分割可用)
        max_samples: 最大处理样本数，None表示处理所有样本
        vocab_size: 词汇表大小限制（不包含BOS/EOS），None表示包含所有唯一词汇
    
    Returns:
        list: 按频率排序的词汇列表（不包含BOS/EOS特殊符号）
    """
    print(f"Loading COCO {split} split...")
    try:
        ds = load_coco(split)
        print(f"Loaded {len(ds)} samples from COCO {split}")
    ...
    if vocab_size is not None:
        # 限制词汇表大小
        most_common = [word for word, freq in counter.most_common(vocab_size)]
        print(f"Built vocabulary with {len(most_common)} words (limited to {vocab_size})")
        if len(counter) > vocab_size:
            print(f"Note: {len(counter) - vocab_size} less frequent words were excluded")
    else:
        # 不限制词汇表大小
...
def save_vocabulary(vocab_list, output_path):
    ...
    # vocab_list中只包含实际词汇，不包含BOS/EOS特殊符号
    with open(output_path, 'w', encoding='utf-8') as f:
        for word in vocab_list:
            f.write(f"{word}\n")
    
    print(f"Vocabulary saved to: {output_path}")
    print(f"Vocabulary size: {len(vocab_list)} words (excluding BOS/EOS)")
    print(f"Note: Index 0 is reserved for EOS token '.', actual words start from index 1")
    print(f"Total model vocab size will be: {len(vocab_list) + 1} (including EOS at index 0)")


def main():
    parser = argparse.ArgumentParser(description='Generate COCO vocabulary file')
    parser.add_argument('--split', default='train', choices=['train'],
                        help='Dataset split to use (COCO only has train split available)')
    ...
    parser.add_argument('--vocab_size', type=int, default=9487,
                        help='Maximum vocabulary size (default: 9487). This excludes BOS/EOS tokens. '
                             'The actual model vocab size will be vocab_size + 1 (for EOS at index 0)')
    parser.add_argument('--output', default='data/coco/coco_vocabulary.txt',
                        help='Output vocabulary file path (default: data/coco/coco_vocabulary.txt)')
    ...
    print("=" * 60)
    print("COCO Vocabulary Generator")
    print("=" * 60)
    print(f"Split: {args.split}")
    print(f"Vocabulary size: {vocab_size if vocab_size else 'UNLIMITED (all unique words)'}")
