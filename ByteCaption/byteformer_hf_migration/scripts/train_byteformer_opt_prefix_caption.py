"""
ByteFormer + OPT Prefix Caption Training Script
使用ByteFormer作为encoder，OPT作为decoder，使用prefix prompt方式实现图像描述生成任务

示例运行命令：
python byteformer-hf-migration/scripts/train_byteformer_opt_prefix_caption.py --per_device_train_batch_size 48 --per_device_eval_batch_size 48 --num_train_epochs 5 --learning_rate 5e-5 --eval_steps 40 --logging_steps 50 --save_steps 600 --lr_scheduler_type cosine --gradient_accumulation_steps 2 --report_to none --max_caption_length 16 --num_eval_samples 50 --fp16 --opt_model facebook/opt-125m
"""
...
from transformers import OPTForCausalLM, OPTConfig, AutoTokenizer, GenerationConfig
...
class PrefixLMForCaption(PreTrainedModel, GenerationMixin):
    """Prefix LM模型，使用ByteFormer encoder + OPT decoder"""
    ...
    def __init__(self, encoder, decoder, decoder_tokenizer, bos_token_id):
        ...
        proj_out = getattr(decoder.config, 'hidden_size', None)  # OPT使用hidden_size而不是n_embd
    ...
    def forward(
        ...
        **kwargs,
        ...
        if labels is not None:
            ...
            labels_for_embeds = self.prepare_decoder_input_ids_from_labels(labels)
            labels_for_embeds = labels_for_embeds.masked_fill(labels_for_embeds == -100, pad_token_id)

            # 使用替换后的 labels 计算词向量 - OPT使用不同的embedding路径
            if hasattr(self.decoder, 'model') and hasattr(self.decoder.model, 'decoder'):
                # OPT结构: model.decoder.embed_tokens
                label_embeds = self.decoder.model.decoder.embed_tokens(labels_for_embeds)
            else:
                # 备用路径
                label_embeds = self.decoder.get_input_embeddings()(labels_for_embeds)
        ...
        if labels is not None:
            ...
            prediction_logits = logits[:, prefix_len:prefix_len+label_len, :]  
    ...
    def generate(self, input_ids=None, encoder_outputs=None, **kwargs):
        ...
        # 获取start token embeddings - 适配OPT结构
        if hasattr(self.decoder, 'model') and hasattr(self.decoder.model, 'decoder'):
            start_embeds = self.decoder.model.decoder.embed_tokens(start_tokens)
        else:
            start_embeds = self.decoder.get_input_embeddings()(start_tokens)
        ...
        for _ in range(max_length - 1):  # -1 because we already have start token
            ...
            # 准备下一轮的输入 - 适配OPT结构
            if hasattr(self.decoder, 'model') and hasattr(self.decoder.model, 'decoder'):
                next_token_embeds = self.decoder.model.decoder.embed_tokens(next_token)
            else:
...
def parse_args():
    """解析命令行参数"""
    parser = argparse.ArgumentParser(description="ByteFormer + OPT Prefix Caption Training")
    ...
    parser.add_argument("--output_dir", type=str, default="./checkpoints/byteformer_opt_prefix_caption", help="训练输出目录")
    ...
    parser.add_argument("--use_lora", action="store_true", default=False, help="使用LoRA微调")
...
def main():
    ...
    print("ByteFormer + OPT Prefix Caption Training")
    print("=" * 50)
    ...
    def convert_bin_to_safetensors(model_name, safe_dir):
        ...
        # 创建模型配置
        opt_config = OPTConfig.from_pretrained(model_files_dir)
        
        # 创建模型实例（不加载权重）
        temp_model = OPTForCausalLM(opt_config)
    ...
    if not os.path.exists(safe_model_dir):
        try:
            ...
            # 保存 tokenizer 配置
            tokenizer_temp = AutoTokenizer.from_pretrained(args.opt_model)
            tokenizer_temp.save_pretrained(safe_model_dir)
            del temp_model
            del tokenizer_temp
            torch.cuda.empty_cache()
    else:
        print(f"Using existing safetensors model: {safe_model_dir}")
    
    # 从本地 safetensors 加载
    print(f"Loading OPT model from safetensors: {safe_model_dir}")
    opt_config = OPTConfig.from_pretrained(safe_model_dir)
    opt_decoder = OPTForCausalLM.from_pretrained(
        safe_model_dir,
        config=opt_config,
    ...
    # 创建tokenizer
    tokenizer = AutoTokenizer.from_pretrained(safe_model_dir)
    # OPT使用</s>作为pad token
    ...
    model = PrefixLMForCaption(encoder, opt_decoder, tokenizer, bos_token_id)
    ...
    # 使用LoRA微调（可选）
    if args.use_lora:
        print(f"启用LoRA微调: r={args.lora_r}, alpha={args.lora_alpha}, dropout={args.lora_dropout}")
        ...
        lora_config = LoraConfig(
            ...
            target_modules=["q_proj", "k_proj", "v_proj", "out_proj", "fc1", "fc2"],  # OPT的attention和FFN层
        )
        ...
        def evaluate(self, eval_dataset=None, desc="Eval", ignore_keys=None, metric_key_prefix: str = "eval"):
            ...
            for batch in eval_iterator:
                ...
                with torch.no_grad():
                    ...
                    start_embeds = self.model.decoder.model.decoder.embed_tokens(start_tokens)
                    ...
                    for step in range(max_length - 1):  # 最多生成max_length-1个新token
                        ...
                        # 准备下一轮的输入
                        next_token_embeds = self.model.decoder.model.decoder.embed_tokens(next_token)
                        current_embeds = torch.cat([current_embeds, next_token_embeds], dim=1)
                        current_attention_mask = torch.cat([
                            current_attention_mask,
                            torch.ones((batch_size, 1), dtype=torch.long, device=device)
    ...
    test_input = prefix_caption_collate_fn([test_sample])
    ...
    with torch.no_grad():
        ...
        print(f"原始caption: {test_sample['caption']}")
        print(f"生成caption: {generated_text}")
