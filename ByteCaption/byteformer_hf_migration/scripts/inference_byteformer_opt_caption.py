"""
ByteFormer + OPT Caption Inference Script
使用训练好的ByteFormer-OPT模型对测试集进行推理生成caption

示例运行命令：
python byteformer-hf-migration/scripts/inference_byteformer_opt_caption.py \
    --model_path /root/autodl-tmp/corenet/_trained_models/byteformer_opt_caption/checkpoint-1000 \
    --num_samples 100 \
    --batch_size 48 \
    --num_beams 5 \
    --max_length 16 \
...
def parse_args():
    """解析命令行参数"""
    parser = argparse.ArgumentParser(description="ByteFormer + OPT Caption Inference")
    ...
    parser.add_argument("--output_file", type=str, default="inference_results_opt.json", help="推理结果输出文件")
...
def load_model_and_tokenizer(model_path: str, config_path: str, weights_path: str, device: str):
    ...
    opt_decoder = CustomOPTForCausalLM.from_pretrained("facebook/opt-125m", config=opt_config)
    ...
    try:
        ...
        print("Failed to load tokenizer from trained model, using facebook/opt-125m tokenizer")
...
def generate_captions(model, tokenizer, dataloader, args, device):
    ...
    for batch_idx, batch in enumerate(dataloader):
        ...
        # 计算损失 - 对ground truth caption进行编码
        try:
            # 编码ground truth captions
            ...
            # 计算损失 (使用teacher forcing)
            with torch.no_grad():
                model.train()  # 临时切换到训练模式以计算损失
                outputs = model(
                    input_ids=input_ids,
                    decoder_input_ids=encoded_captions["input_ids"][:, :-1],  # 去掉最后一个token作为输入
                    labels=encoded_captions["input_ids"][:, 1:]  # 去掉第一个token作为标签
                )
            ...
            print(f"Warning: Failed to compute loss for batch {batch_idx}: {e}")
...
def main():
    ...
    print("ByteFormer + OPT Caption Inference")
    print("=" * 50)
